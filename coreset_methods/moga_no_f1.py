...
class MOGA(BaseEstimator, TransformerMixin):
    def __init__(self, x, y, classifier, metric, solutionSize = 1, sizePop = 100, numGen = 100, cxPb = 0.8, indMPb = 1, mutPb = 0.07, 
        ...
        if coreset:
            if not (size == 1.0 or size == -1.0): # size fixed
                ...
                if distance and (classDist is not None):
                    creator.create("Fitness", base.Fitness, weights = (1.0, -1.0,)) #(max distance +/-, min -entropy)
                elif distance:
                    creator.create("Fitness", base.Fitness, weights = (size, 1.0,)) #(max performance, max/min size(-1 by default), max distance +/-)
                elif classDist is not None:
                    creator.create("Fitness", base.Fitness, weights = (size, -1.0,)) #(max performance, max/min size(-1 by default), min classDist(min difference of ratio or baseline))
                else:
    ...
    def transform(self, X, y=None):
        ...
        if self.distance and (self.classDist is not None):
            sorted_pop = sorted(pop, key=lambda ind: ind.fitness.values[1], reverse=False) #fixme
        elif self.distance:
            sorted_pop = sorted(pop, key=lambda ind: ind.fitness.values[1], reverse=True)
        elif self.classDist is not None:
            sorted_pop = sorted(pop, key=lambda ind: ind.fitness.values[1], reverse=False)
        else:
...
def fitness_evaluation(ind, classifier, metric,  X_train, X_validate, y_train, y_validate, size, coreset, distance, classDist, purity, seed, num_classes):
    ...
    if coreset:
            if not (size == 1.0 or size == -1.0): # size fixed
                ...
                if distance and (classDist is not None):
                    return (distance_class(core_x, core_y), manipulate_distribution(core_y, y_train, purity, classDist),)
                elif distance:
                    return (len(indices), distance_class(core_x, core_y),)
                elif classDist is not None:
                    return (len(indices), manipulate_distribution(core_y, y_train, purity, classDist),)
                else:
